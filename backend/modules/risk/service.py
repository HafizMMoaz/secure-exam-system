...
def stream_events(exam_id):
    """
    Server-Sent Events stream of monitoring events for one exam.

    Polls the four monitoring collections every 2 seconds for new events
    (filtered by exam_id, using _id as a high-water mark) and yields
    each as a `data: <json>\\n\\n` line in the SSE wire format.

    Frontend consumes via EventSource. This is the §24 bonus
    "real-time monitoring" feature implemented on top of Flask without
    requiring a WebSocket library.
    """
    ...
    from config.config import (
        tab_events_col,
        clipboard_events_col,
        activity_events_col,
        behavioral_events_col,
    )

    streams = [
        ("tab_event", tab_events_col),
        ("clipboard_event", clipboard_events_col),
        ("activity_event", activity_events_col),
        ("behavioral_event", behavioral_events_col),
    ]

    # High-water mark per collection; only emit docs whose _id is newer
    # than what we've already seen. Initialize to the latest _id at
    # stream open to avoid replaying history.
    high_water = {}
    for kind, col in streams:
        latest = col.find_one({"exam_id": exam_id}, sort=[("_id", -1)])
        high_water[kind] = latest["_id"] if latest else ObjectId("000000000000000000000000")
    ...
    yield f"event: stream_open\ndata: {_json.dumps({'exam_id': exam_id})}\n\n"

    while True:
        for kind, col in streams:
            try:
                cursor = col.find(
                    {"exam_id": exam_id, "_id": {"$gt": high_water[kind]}}
                ).sort("_id", 1).limit(50)
                for doc in cursor:
                    high_water[kind] = doc["_id"]
        ...
        _time.sleep(2)
        # Keep-alive comment so proxies do not close the connection.
        yield ": keep-alive\n\n"
